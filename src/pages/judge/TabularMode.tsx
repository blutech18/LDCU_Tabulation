...
import ScoringTabular, {
  ScoringTabularRef,
} from "../../components/judge/ScoringTabular";
import RankingTabular, {
  RankingTabularRef,
} from "../../components/judge/RankingTabular";
...
const TabularMode = () => {
  ...
  const [allowedParticipantIds, setAllowedParticipantIds] = useState<
    number[] | null
  >(null);
  ...
  const fetchCategory = async () => {
    ...
    if (!error && data) {
      ...
      if (limit && limit > 0) {
        const topIds = await calculateTopParticipantIds(
          cat.events.id,
          limit,
          cat.events.participant_type,
        );
        ...
        if (isRankingBased) {
          ...
          sorted.forEach((item) => {
            if (item.avgRank !== null) {
              ...
              participantCategoryValues[item.participant.id][category.id] =
                currentRank;
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div
          className={`w-10 h-10 border-4 rounded-full animate-spin ${isDarkMode ? "border-white/20 border-t-white" : "border-maroon/20 border-t-maroon"}`}
        />
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <button
            onClick={handleBack}
            className={`font-medium ${isDarkMode ? "text-gold hover:text-gold-light" : "text-maroon hover:text-maroon-dark"}`}
          >
            Go Back
          </button>
        </div>
      ...
      >
        ...
        <div className="flex-1 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <h1
              className={`text-3xl font-bold ${isDarkMode ? "text-white" : "text-maroon"}`}
            >
              {category.name}
            </h1>
            <span
              className={`text-lg ${isDarkMode ? "text-white/40" : "text-gray-400"}`}
            >
              •
            </span>
            <p
              className={`text-lg ${isDarkMode ? "text-white/60" : "text-gray-600"}`}
            >
          ...
          <div className="flex items-center gap-3">
            <span
              className={`inline-flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium ${isDarkMode ? "bg-maroon/90 border border-maroon" : "bg-maroon border border-maroon-dark"}`}
            >
              {isRankingBased ? (
                <>
