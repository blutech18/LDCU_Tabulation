...
const AuditorLogin = () => {
    ...
    const [isDarkMode, setIsDarkMode] = useState(() => {
        const saved = localStorage.getItem('auditorDarkMode');
        return saved === 'true';
    });
    const navigate = useNavigate();
    const { setAuditor } = useAuthStore();

    const toggleDarkMode = () => {
        const newMode = !isDarkMode;
        setIsDarkMode(newMode);
        localStorage.setItem('auditorDarkMode', String(newMode));
    };
    ...
    const handleLogin = async (e: React.FormEvent) => {
        ...
        const { data: auditor, error: fetchError } = await supabase
            .from('auditors')
            .select('*')
        ...
        if (fetchError || !auditor) {
            setError('Invalid access code. Please try again.');
            setLoading(false);
            return;
        }

        // Save auditor to store
        setAuditor(auditor);
        navigate('/auditor/results');
    };
            ...
            >
                ...
                <div className={`rounded-3xl p-8 shadow-2xl ${isDarkMode ? 'bg-white/10 backdrop-blur-xl border border-white/20' : 'bg-white border border-gray-200'}`}>
                    ...
                    <div className="text-center mb-8">
                        ...
                        <h1 className={`text-2xl font-bold ${isDarkMode ? 'text-white' : 'text-maroon'}`}>Auditor Panel</h1>
                        <p className={`mt-1 ${isDarkMode ? 'text-white/60' : 'text-gray-600'}`}>Enter your access code to view results</p>
                    </div>
